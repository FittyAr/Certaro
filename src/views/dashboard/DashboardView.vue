...
const mostrarOnboarding = computed(() => {
  if (ui.onboardingDismissed) return false
  if (!stats.value) return false
  return (
    stats.value.cantidadMovimientos === 0 &&
    (!stats.value.topClientes || stats.value.topClientes.length === 0)
  )
})
