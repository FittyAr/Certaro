...
use eo_application::config::{AppConfig, BusinessConfig, LocaleConfig, ReportConfig};
...
pub(crate) fn io_error(operacion: &str, cause: impl std::fmt::Display) -> eo_application::AppError {
    eo_application::AppError::io(anyhow::anyhow!("{operacion}: {cause}"))
}
