...
pub async fn verify(db: &DatabaseTransaction, report: &mut ImportReport) -> Result<()> {
    ...
    if report.has_blocking_issues() {
        anyhow::bail!("verification found {} blocking issues", report.blocking_issues.len());
    }
...
async fn verify_invariants(db: &DatabaseTransaction, report: &mut ImportReport) -> Result<()> {
    ...
    let inv1 = query_count(db, "SELECT COUNT(*) as cnt FROM facturas WHERE total <> subtotal + iva AND is_deleted = 0").await?;
    if inv1 > 0 {
        report.block(format!("invariant 1: {inv1} invoices with total != subtotal + iva"));
    }
    ...
    if inv2 > 0 {
        report.block(format!("invariant 2: {inv2} movements without tipo_movimiento_id"));
    }

    // 5. movimientos.cantidad never zero.
    let inv5 = query_count(db, "SELECT COUNT(*) as cnt FROM movimientos WHERE cantidad = 0 AND is_deleted = 0").await?;
    if inv5 > 0 {
        report.block(format!("invariant 5: {inv5} movements with cantidad = 0"));
    }

    // 6. Multipliers never zero.
    for col in &["multiplicador_sabado", "multiplicador_domingo", "multiplicador_feriado"] {
        let inv6 = query_count(db, &format!("SELECT COUNT(*) as cnt FROM liquidaciones WHERE {col} = 0 AND is_deleted = 0")).await?;
        if inv6 > 0 {
    ...
    if inv7 > 0 {
        report.block(format!("invariant 7: {inv7} clients with multiple principal contacts"));
    }

    // 9. row_version is 8 bytes.
    for table in &["movimientos", "facturas", "clientes", "obras", "trabajos", "empleados"] {
        let inv9 = query_count(db, &format!("SELECT COUNT(*) as cnt FROM {table} WHERE LENGTH(row_version) <> 8 AND is_deleted = 0")).await?;
        if inv9 > 0 {
            report.block(format!("invariant 9: {inv9} rows in {table} with invalid row_version length"));
        }
    }
