...
use eo_domain::entities::{Audit, Trabajo};
use eo_domain::{EstadoObra, EstadoTrabajo, StateMachine};
use tracing::info;
use uuid::Uuid;
...
async fn cargar_obra(tx: &dyn Transaction, id: Uuid) -> AppResult<eo_domain::entities::Obra> {
    tx.obras()
        .find_by_id(id)
        .await?
