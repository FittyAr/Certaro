...
pub fn validate(input: &CategoriaInput, id: Option<Uuid>) -> AppResult<()> {
    ...
    v.required_text("nombre", &input.nombre, "Validation.Categoria.NombreRequired");
    v.max_length(
        "nombre",
        &input.nombre,
...
mod tests {
    ...
    fn el_nombre_es_obligatorio() {
        let error = validate(&CategoriaInput { nombre: "  ".into(), ..input() }, None).unwrap_err();
    ...
    fn el_color_acepta_solo_seis_digitos_hexadecimales_con_numeral() {
        for valido in ["#FFAA00", "#000000", "#abcdef"] {
            let dto = CategoriaInput { color_hex: Some(valido.into()), ..input() };
            assert!(validate(&dto, None).is_ok(), "{valido}");
        }
        for invalido in ["FFAA00", "#FFF", "#GGGGGG", "#FFAA000"] {
            let dto = CategoriaInput { color_hex: Some(invalido.into()), ..input() };
            assert_eq!(
    ...
    fn un_color_vacio_es_ausencia_de_color_y_no_un_error() {
        let dto = CategoriaInput { color_hex: Some("   ".into()), ..input() };
    ...
    fn una_categoria_no_puede_ser_su_propio_padre() {
        ...
        let dto = CategoriaInput { categoria_padre_id: Some(id), ..input() };
        assert_eq!(
