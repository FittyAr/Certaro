...
pub fn nombre_backup(now: chrono::DateTime<chrono::Utc>) -> String {
    format!("certaro_{}.db", now.format("%Y%m%d_%H%M%S"))
}
...
pub fn instante_de(nombre: &str) -> Option<chrono::DateTime<chrono::Utc>> {
    let resto = nombre.strip_prefix("certaro_")?.strip_suffix(".db")?;
...
mod tests {
    ...
    fn el_nombre_va_y_vuelve() {
        ...
        assert_eq!(nombre, "certaro_20260829_143012.db");
    ...
    fn un_archivo_ajeno_no_se_interpreta_como_backup() {
        for nombre in [
            ...
            "certaro_20260829_143012.sqlite",
            "certaro_ayer.db",
        ] {
